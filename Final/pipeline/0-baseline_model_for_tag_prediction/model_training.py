...
def model_train(feature_data, model, n_est=15, max_dep=5):
...
def main(argv=None):
    ...
    usage = "Usage: %prog input_feature.csv output_model_name (pass -h for more info)"
    ...
    if len(args) < 2:
        parser.error("Must pass in a feature file and a output model name")

    if options.max_depth is not None and options.n_estimator is not None:
        model_train(
            args[0],
            args[1],
            n_est=options.n_estimator,
            max_dep=options.max_depth)
    elif options.n_estimator is not None:
        model_train(args[0], args[1], n_est=options.n_estimator)
    elif options.max_depth is not None:
        model_train(args[0], args[1], max_dep=options.max_depth)
    else:
        model_train(args[0], args[1])
    return 0
