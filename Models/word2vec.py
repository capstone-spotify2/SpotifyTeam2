...
def mostSimilarFromList(inputw, wlist, wmodel):
    ...
    if inputw_ in vocabs:
        ...
        sims = []
        for w in wlist:
            sims.append(np.mean([wmodel.similarity(iw, w)
                                 for iw in inputw if iw in wmodel.vocab.keys()]))
        return wlist[np.argmax(sims)]
    else:
...
lfiles = [
    dir +
    f for f in listdir(dir) if isfile(
        join(
            dir,
            f)) and f.endswith(".txt")]

for i, f in enumerate(lfiles[486 +
                             521 +
                             4 +
                             18 +
                             7 +
...
print("There are " + str(len(sentences)) +
      " sentences in our corpus of opeds.")
...
#     songw[i] = pd.Series(
#         list(filter(lambda x: x not in stop, song[song != 0].index[1:])))
...
output = [pd.Series([findDistance(words, 'happy', modeldf), findDistance(
    words, 'sad', modeldf)]) for k, words in songw.iteritems()]
